...
benötigen. Die Installation von Sensoren wird als eine Aufgabe mit zwei
Dimensionen betrachtet: Platzierung und Assoziation. Dies sind wichtige
Anhaltspunkte um eine korrekte Installation definieren zu können und um die
Randbedingugen des Modells zu definieren.
...
Anbringungsreihenfolge, auf die der Nutzer acht geben musste.
...
ein wichtiger Faktor der die Nutzbarkeit dieses Ansatzes stark verbessert. 
...
\begin{table}[htb]
  ...
  \begin{tabular}{l c c c c}
   ...
   & AR-Handbook & \dopler & LiMonE & Gendoc \\  [1ex]
    ...
    %     people \\ porduct managers} \\ [1ex]
     ...
     \rownumber.Eignung im Szenario & ++ & x & - & -   \\ [1ex]

     \rownumber.Verlangtes Expertenwissen & ++ & x & - & -
     \\[1ex]

     \rownumber.Verhalten bei Veränderung des Systems & ++ & x & - & - \\ [1ex]
...
Vergleichsfaktoren:
Eignung Szenario
Verlangtes Expertenwissen in der Benutzung

Verhalten bei Veränderung des Systems
	dopler: ok, aber wenn sich die KOmponenten die statische textfragmente
	zugeordnet haben, muss manuell nachgebessert werden
	vr: neuaufnahme des veränderten teils und 
	
Struktur modellierung/dokumentierung



zusammenhänge darstellen
\begin{description}
  \item[LiMonE] Literate Modelling
  \item[TopCased] UML might suffer from trivialisation of Requirements
  \item[Dopler] DocBook Markup als  
\end{description}
verhalten
...
Unterschied in der Erstellung der Tempaltes bei Dopler und TopCased
\begin{description}
  \item[NL] Grammar based oder einfache Templates (mit einer abfragesprache?)
  \item[TopCased] Skript direkt in ODT oder DOCX (mit custom tags?)
  \item[Dopler] DocBook Markup als  
\end{description}
...
Dopler higher level than Topcased aber lower level als AR und Limone?
...
Anwendungsgebiete
\begin{description}
  \item[LiMonE] ?? wurde in einer Masterarbeit erstellt
  \item[TopCased] Konsortium von Industriepartnern in der aktiven Entwicklung
  von kritischen Anwendungen
  \item[Dopler] Industrielle Anwendung bei Siemens VAI und Siemens AG
\end{description}

Generalisierbarkeit (Erweiterbarkeit auf andere Domänen(versch. Meta-Modelle))
großer/kleiner Aufwand

Topcased: alle von Papyrus unterstützten Diagramme + zusätzliche
LiMonE: nur Klassendiagramme (also keine dynamischen Aspekte)

Visuelle Unterstützung durch Einbindung von graphischen Elementen 
\begin{description}
  \item[LiMonE] ?? beschränkt auf Klassendiagramme und die synchronisation
  bricht wahrscheinlich?!
  \item[TopCased] Automatisches Einfügen eines Renderings der Systemmodelle
  \item[Dopler] Manuell erstellte Dokumentenfragmente
\end{description}


Weitere nicht qualitative Unterscheidungskriterien (Unterschiede)

Ziele der generierten Artefakte
\begin{description}
  \item[LiMonE] Requirements Dokumente
  \item[TopCased] Architekturbeschreibung
  \item[Dopler] Produktbeschreibungsdokumente
\end{description}

Open Source vs Proprietär vs nicht Released
